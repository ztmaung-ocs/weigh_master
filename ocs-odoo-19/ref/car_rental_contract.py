...
class DashboardFleetRental(models.Model):
    ...
    def car_details(self):
        ...
        running_cars = self.env['car.rental.contract'].search(
            [('state', '=', 'running')], order='rent_end_date asc')
        cars_available = self.env['fleet.vehicle'].search(
            [('rental_check_availability', '=', True)], order='name asc')
        running_details = []
        available_cars = []
        for record in running_cars:
            running_details.append({'vehicle':  f"{record.vehicle_id.model_id.brand_id.name}/{record.vehicle_id.model_id.name}   {record.vehicle_id.license_plate}",
                                    'rent_name' : record.name,
        ...
        for record in cars_available:
            available_cars.append({
                'available_car': f"{record.model_id.brand_id.name}/{record.model_id.name}    {record.license_plate}",
            })
        values = {
